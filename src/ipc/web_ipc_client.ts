...
export class WebIpcClient {
  ...
  public async createApp(params: CreateAppParams): Promise<CreateAppResult> {
    ...
    return { app, chatId } as CreateAppResult;
  }
  ...
  public async streamMessage(
    ...
    try {
      ...
      chat.messages.push({ id: assistantMsgId, role: "assistant", content: "" });
      this.writeChats(chats);
      options.onUpdate(chat.messages);
  ...
  public async addDependency(_args: { chatId: number; packages: string[] }): Promise<void> {}
  ...
  public onGithubDeviceFlowUpdate(_cb: any): () => void { return () => {}; }
  public onGithubDeviceFlowSuccess(_cb: any): () => void { return () => {}; }
  public onGithubDeviceFlowError(_cb: any): () => void { return () => {}; }
  ...
  public async createGithubRepo(_org: string, _repo: string, _appId: number): Promise<void> {}
  public async syncGithubRepo(_appId: number): Promise<{ success: boolean; error?: string }> {
    return { success: false, error: "Not supported on web" };
  }
  public async disconnectGithubRepo(_appId: number): Promise<void> {}

  public async getAppVersionWeb(): Promise<string> { return "web"; }

  public async getProposal(_chatId: number) { return null as any; }
  public async approveProposal(_args: { chatId: number; messageId: number }) { return {} as any; }
  public async rejectProposal(_args: { chatId: number; messageId: number }) {}

  public async listSupabaseProjects(): Promise<any[]> { return []; }
  public async setSupabaseAppProject(_project: string, _app: number): Promise<void> {}
  ...
  public async uploadToSignedUrl(_url: string, _contentType: string, _data: any): Promise<void> {}
  public async listLocalOllamaModels() { return []; }
  public async listLocalLMStudioModels() { return []; }

  public onDeepLinkReceived(_callback: (data: { type: string; url?: string }) => void) { return () => {}; }
  ...
  public async doesReleaseNoteExist(_params: DoesReleaseNoteExistParams): Promise<{ exists: boolean; url?: string }> {
    return { exists: false };
  }
  ...
  public async getLanguageModels(params: { providerId: string }): Promise<LanguageModel[]> {
    const res = await fetch(`/api/models?provider=${encodeURIComponent(params.providerId)}`);
    if (!res.ok) return [];
    const models = (await res.json()) as string[];
    return models.map((name) => ({ id: name, apiName: name, displayName: name })) as any;
  }
  ...
  public async getLanguageModelsByProviders(): Promise<Record<string, LanguageModel[]>> {
    ...
    const entries = await Promise.all(
      providers
        ...
        .map(async (p) => [p.id, await this.getLanguageModels({ providerId: p.id })] as const),
    );
  ...
  public async deleteCustomModel(_params: { providerId: string; modelApiName: string }): Promise<void> {}
  public async deleteCustomLanguageModelProvider(_providerId: string): Promise<void> {}

  public async selectAppFolder(): Promise<{ path: string | null; name: string | null }> {
    return { path: null, name: null };
  }
  public async checkAiRules(_params: { path: string }): Promise<{ exists: boolean }> { return { exists: false }; }
  ...
  public async checkAppName(_params: { appName: string }): Promise<{ exists: boolean }> { return { exists: false }; }
